...
If this workspace has no configuration, or the user says this is the first run, read `references/onboarding-guide.md` and run the setup wizard before doing a full review.
...
8. **Export approval results.** Notes typed in a local page are not available until the user exports or sends JSON/text back.
