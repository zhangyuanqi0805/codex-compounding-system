...
```text
请使用 skill-installer 从 https://github.com/zhangyuanqi0805/ai-compounding-system/tree/main/skills/ai-compounding-system 安装 AI 复利系统 Skill。安装完成后提醒我重启 Codex；重启后请启动 AI 复利系统的首次配置向导，逐个问题问我完成个性化设置，并先跑一个指定日期的小样。
```
...
```text
Use $skill-installer to install the skill from https://github.com/zhangyuanqi0805/ai-compounding-system/tree/main/skills/ai-compounding-system. After installation, remind me to restart Codex. Then use $ai-compounding-system to run the first-time setup wizard step by step.
```

如果当前 Codex 没有 `skill-installer`，可以让 Codex 手动安装：

```text
请下载 https://github.com/zhangyuanqi0805/ai-compounding-system，把其中的 skills/ai-compounding-system 文件夹复制到我的 Codex Skills 目录；复制完成后提醒我重启 Codex，再启动首次配置向导。
```
...
1. 你的数字资产库在哪里？如果还没有，可以先只生成入库建议。
2. 你的工作日按哪个时区计算？默认用你的本地时区。
3. 我能看哪些材料？默认看 Codex/Code Desk 工作记录和本地过程产物，云文档只记录链接。
4. 要不要以后每天自动跑？首次默认不开自动化，先跑一天小样。
5. 你想先复盘哪一天？推荐先用昨天或指定的一天。
...
审批方式默认就是本地 HTML 审批台：可以点选动作、写备注、导出 JSON。新手不需要在 Markdown 清单、聊天总结和网页审批台之间做选择。
...
看完审批台后，把导出的 JSON 或你的审批意见发回 Codex，让它执行被批准的动作，并把偏差写进校准档案。
